...
export const BanquetOptionPage: React.FC = (): JSX.Element => {
    ...
    const getTotalPrice = () => {
        return Math.round(
            (1 + (banquetOption?.service_fee || 0) / 100) *
    ...
    const getMinTimeForStart = () => {
        if (date) {
            ...
            if (timeTo && timeTo.value !== 'до' && max_duration && max_duration > 0) {
                ...
                const minStart = moment(timeTo.value as string, 'HH:mm')
                    .subtract(max_duration, 'hours')
                    .format('HH:mm');
                const minStartIsTheDayBefore =
                    moment(minStart, 'HH:mm').isAfter(moment(timeTo.value as string, 'HH:mm')) ||
                    moment(timeTo.value as string, 'HH:mm').isAfter(moment(dayEnd as string, 'HH:mm'));
    ...
    const getMaxTimeForStart = () => {
        return date
            ? timeTo.value !== 'до'
                ? subtractOneHour(timeTo.value as string)
    ...
    const getMinTimeForEnd = () => {
        return date
            ? timeFrom.value !== 'с'
                ? addOneHour(timeFrom.value as string)
    ...
    const getMaxTimeForEnd = () => {
        if (date) {
            ...
            if (timeFrom && timeFrom.value !== 'с' && max_duration && max_duration > 0) {
                ...
                const maxEnd = moment(timeFrom.value as string, 'HH:mm')
                    .add(max_duration, 'hours')
                    .format('HH:mm');
                const maxEndIsAfterMidnight =
                    moment(maxEnd, 'HH:mm').isBefore(moment(timeFrom.value as string, 'HH:mm')) ||
                    moment(timeFrom.value as string, 'HH:mm').isBefore(moment(dayStart, 'HH:mm'));
            ...
            <div className={css.page}>
                ...
                <div className={css.pageWrapper}>
                    <div className={css.header}>
                        ...
                        <span className={css.header_title}>{banquetOption?.name}</span>
                        <div style={{ width: 40 }} />
                    </div>
                    <ContentContainer>
                        <ContentBlock>
                            <div className={css.formContainer}>
                                <div className={css.datePicker} onClick={() => setCalendarOpen(true)}>
                                    ...
                                    {date ? (
                                        ...
                                        <span className={css.datePicker__placeholder}>Желаемая дата</span>
                                    )}
                                </div>
                                ...
                                <div className={css.guestCount} onClick={() => setOpenPopup(true)}>
                                    <UsersIcon size={24} />
                                    {guestCount.value === 'unset' ? (
                                        <span className={css.guestCount__placeholder}>Количество гостей</span>
                                    ) : (
                        ...
                        {isFormValid && banquetOption?.deposit !== null && (
                            <ContentBlock>
                                <div className={css.payment}>
                                    <span className={css.payment_title}>Предварительная стоимость*:</span>
                                    <div className={css.payment_text}>
                                        <span>Депозит на человека:</span>
                                        <span>{banquetOption?.deposit} ₽</span>
                                    </div>
                                    <div className={css.payment_text}>
                                        ...
                                        <span>
                                            {banquetOption &&
                                                guestCount.value !== 'unset' &&
                                                Number(banquetOption?.deposit) * parseInt(guestCount.value as string) +
                                                    ' ₽'}
                                        </span>
                                    </div>
                                    <div className={css.payment_text}>
                                        <span>Сервисный сбор:</span>
                                        <span>{banquetOption?.service_fee}%</span>
                                    </div>
                                    <div className={css.payment_text}>
                                        <span>Итого:</span>
                                        <span>{banquetOption && getTotalPrice() + ' ₽'}</span>
                                    </div>
                                    <p>
