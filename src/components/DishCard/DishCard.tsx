...
export const DishCard: React.FC<DishCardProps> = ({
    ...
    return (
        <div className={css.menuItem}>
            <div className={css.image} style={{ backgroundImage: `url(${image_url})` }} onClick={onClick} />
            <div className={css.content} onClick={onClick}>
                <div className={css.subtitle}>
                    <span className={css.title}>{guest_title}</span>
                    {defaultWeight && <span className={css.weight}>{defaultWeight}</span>}
                </div>
            </div>
            <div className={quantity > 0 ? css.buttonActive : css.button} onClick={() => quantity === 0 && onAdd(id)}>
                <div className={css.iconSummary}>
                    {quantity > 0 ? (
                        <>
                            ...
                            <span className={css.priceText}>{quantity}</span>
                            <button
                                className={css.iconButton}
                                onClick={(e) => {
                                    e.stopPropagation();
                        ...
                        <>
                            <span className={css.priceText}>{defaultPrice} ₽</span>
