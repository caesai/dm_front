...
import { ISubscription, IUser, IUserPreferences, IUserUpdate } from '@/types/user.types.ts';

export const APIUpdateUserInfo = async (
    { first_name, last_name, date_of_birth, phone_number, allergies, email }: IUserUpdate,
    access_token: string
) => {
    let payload = {};
...
export const APIDeleteUser = async (access_token: string) => {
    return await axios.post<IUser>(
        `${BASE_URL}/user/delete_me`,
        {},
        {
            headers: {
                Authorization: `Bearer ${access_token}`,
...
export const APIUserPreferences = async (token: string, data: IUserPreferences) => {
    return await axios.post(`${BASE_URL}/user-preferences/user/bulk-update`, data, {
        headers: {
            Authorization: `Bearer ${token}`,
        },
    });
...
export const APIGetUserSubscriptions = async (token: string) => {
    return await axios.get<ISubscription>(`${BASE_URL}/user/subscriptions`, {
        headers: {
            Authorization: `Bearer ${token}`,
        },
    });
...
export const APIPutUserSubscriptions = async (token: string, data: ISubscription) => {
    return await axios.put<ISubscription>(
        `${BASE_URL}/user/subscriptions`,
        {
            subscriptions: data,
        },
        {
